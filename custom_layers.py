...
# Created by junfeng, saj on 5/12/16.
...
def binary_accuracy(probs, y, mask, length_var):
    # (n_samples, n_timesteps_f)
    probs_shp = probs.shape
    predicted = T.ge(probs, 0.5)
    # (n_samples * n_timesteps_f)
    y_flat = y.flatten()
    acc = lasagne.objectives.binary_accuracy(probs.flatten(), y_flat)
    # (n_samples, n_timesteps_f)
    acc = acc.reshape(probs_shp)
    acc = acc * mask
    acc = T.sum(acc, axis=1) / length_var
    return acc, predicted
...
class AnsPointerLayer(lasagne.layers.LSTMLayer):
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        def step(input_i, cell_previous, hid_previous, *args):
            ...
            mh += self.b_a_pointer.dimshuffle('x', 0)
            # mh is (n_batch, 1, n_features)
            mh = mh.dimshuffle(0, 'x', 1)
            ...
            alpha = T.flatten(alpha, 2)
            alpha += self.c_pointer.dimshuffle('x', 0)
            # 0 after softmax is not 0, fuck, my mistake.
